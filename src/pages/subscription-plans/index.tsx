...
type ColumnType = ColumnDef<SubscriptionPlanInterface, unknown>;

interface SubscriptionPlanListPageProps {
...
export function SubscriptionPlanListPage(props: SubscriptionPlanListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SubscriptionPlanInterface>>(
    () => `/subscription-plans?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: SubscriptionPlanInterface) => {
    if (hasAccess('subscription_plan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/subscription-plans/view/${row.id}`);
    }
  };
    ...
    { id: 'description', header: 'Description', accessorKey: 'description' },
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/subscription-plans/view/${record.id}`} passHref legacyBehavior>
            <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="subscription_plan" />
        </Flex>
